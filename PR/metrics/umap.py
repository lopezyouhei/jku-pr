...
def main(device, categories):
    ...
    class_to_classid = load_yaml(os.path.join(data_path, "class_to_classid.yaml"))
    ...
    for model_name, model_path in models.items():
        ...
        # umap embedding
        reducer = umap.UMAP()
        l_umap = reducer.fit_transform(l_embeddings)
        print(type(l_umap))
        z_umap = reducer.fit_transform(z_embeddings)
        p_umap = reducer.fit_transform(p_embeddings)
        
        # plot umap
        plot_umap(l_umap, category_labels, synset_categories, category_names, model_name, 'l')
        plot_umap(z_umap, category_labels, synset_categories, category_names, model_name, 'z')
        plot_umap(p_umap, category_labels, synset_categories, category_names, model_name, 'p')
